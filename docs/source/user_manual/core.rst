...
The main package of the EvoGuessAI component. В ядре описывается логика взаимодействия всех зависимых модулей для выполнения требуемой задачи. Основная задача, которую позволяет решить библиотека EvoGuess, является задача метаэвристической оптимизации целевой black-box function.
